...
var VNKeys = new function () {
    ...
    // ---- Public API additions (patched) ----
    // Re-scan the document for elements marked with [data-vnkeys] and attach handlers.
    this.refresh = function () { setup(); };

    // Enable VNKeys for a specific <input type="text"> element.
    this.enable = function (el) {
        if (!el) return;
        try { el.setAttribute("data-vnkeys", ""); } catch (e) {}
        el.oninput = inputEventHandler;
    };

    // Disable VNKeys for a specific element (removes handler + attribute).
    this.disable = function (el) {
        if (!el) return;
        try { el.removeAttribute("data-vnkeys"); } catch (e) {}
        if (el.oninput === inputEventHandler) el.oninput = null;
    };

    // Convenience: toggle based on boolean
    this.setEnabled = function (el, enabled) {
        if (enabled) this.enable(el);
        else this.disable(el);
    };
    // ---- End public API additions ----
